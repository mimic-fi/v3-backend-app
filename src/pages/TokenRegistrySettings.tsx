...
const TokenRegistrySettings: React.FC = () => {
  const [
    ...
    setTokenRegistrySettings,
  ] = useState<TokenRegistrySetting[] | null>(null);
  ...
  const fetchTokenListSettings = async () => {
    try {
      ...
      const response = await axios.get<TokenRegistrySetting[]>(
      ...
      setTokenRegistrySettings(response.data);
    } catch (error) {
